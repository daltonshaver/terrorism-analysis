...
def totalFrequencyTable(dataset):
    dataset = dataset
    def frequencyTable(dataset, var):
        table = dataset[var].value_counts()
        return table
    
    tab1 = frequencyTable(dataset, 'suicide')
    tab2 = frequencyTable(dataset, 'vicinity')
    tab3 = frequencyTable(dataset, 'gname')
    tab4 = frequencyTable(dataset, 'attacktype1')
    tab5 = frequencyTable(dataset, 'targtype1')
    tab6 = frequencyTable(dataset, 'weaptype1')
    tab7 = frequencyTable(dataset, 'country')
    
    table = tab1.append([tab2, tab3, tab4, tab5, tab6, tab7])
    return table

freq_hoa95 = totalFrequencyTable(prob_hoa95)
freq_p95 = totalFrequencyTable(prob_p95)
freq_ts95 = totalFrequencyTable(prob_ts95)
...
def totalContingencyTable(dataset):
    dataset = dataset
    def contingencyTable(dataset, var):
        table = pd.crosstab(dataset[var], dataset['success'])
        table['percent'] = table[1] / (table[0] + table[1])
        return table
    
    tab1 = contingencyTable(dataset, 'suicide')
    tab2 = contingencyTable(dataset, 'vicinity')
    tab3 = contingencyTable(dataset, 'gname')
    tab4 = contingencyTable(dataset, 'attacktype1')
    tab5 = contingencyTable(dataset, 'targtype1')
    tab6 = contingencyTable(dataset, 'weaptype1')
    tab7 = contingencyTable(dataset, 'country')
    
    table = tab1.append([tab2, tab3, tab4, tab5, tab6, tab7])
    return table

cont_p = totalContingencyTable(oef_p)
cont_hoa = totalContingencyTable(oef_hoa)
cont_ts = totalContingencyTable(oef_ts)
